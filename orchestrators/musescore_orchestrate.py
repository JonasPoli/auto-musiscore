...
sys.path.insert(0, str(__import__('pathlib').Path(__file__).parent.parent / 'utils'))
...
def process_midi_to_8part(input_path, output_path, mapping, use_expression=True, speed=1.0):
    ...
    for idx, conf in enumerate(instruments_config):
        ...
        for track in mid.tracks:
            ...
            # Emparelha note_on / note_off para calcular duração e corrigir staccato
            raw_notes = []
            active_notes_pairs = {}
            for msg in abs_track:
                if msg.is_meta:
                    continue
                if not hasattr(msg, 'channel') or msg.channel != target_channel:
                    continue
                if msg.type == 'note_on' and msg.velocity > 0:
                    active_notes_pairs[msg.note] = (msg.time, msg.velocity)
                elif msg.type == 'note_off' or (msg.type == 'note_on' and msg.velocity == 0):
                    if msg.note in active_notes_pairs:
                        on_t, vel = active_notes_pairs.pop(msg.note)
                        raw_notes.append({"note": msg.note, "on_time": on_t, "off_time": msg.time, "velocity": vel})

            raw_notes.sort(key=lambda x: x["on_time"])
            ...
            for note_info in raw_notes:
                ...
                # Duração original com correção de staccato
                duration = off_t - on_t
                ...
                note_off_msg = mido.Message('note_off', channel=midi_channel, note=note_num, velocity=0,        time=off_t_new)

                # Expressão dinâmica baseada em velocity
                if use_expression:
                    note_on_msg.velocity = calculate_expressive_velocity(
                        note_on_msg, on_t_new, avg_pitch, phrase_boundaries, voice_name, mid.ticks_per_beat
                    )
                else:
                    vel_offset = int(80 * random.uniform(-0.04, 0.04))
                    note_on_msg.velocity = max(1, min(127, 80 + vel_offset))
...
def render_score(midi_path, output_mp3, soundfonts_dir):
    ...
    try:
        ...
        import shutil
        for path in ["automation.json", "audiosettings.json", "viewsettings.json"]:
            if os.path.exists(path):
                try:
                    os.remove(path)
                except Exception:
                    pass
        for folder in ["META-INF", "Thumbnails"]:
...
def main():
    ...
    ap.add_argument("--output",     default="output/musescore", help="Pasta de saída para os arquivos MP3")
