    ...
    for conf in instruments_config:
        ...
        for track in mid.tracks:
            ...
            for i, note_info in enumerate(notes):
                ...
                duration_original = remove_staccato(duration_original, ticks_per_beat)
...
def render_score(midi_path, output_mp3, soundfonts_dir, cc7_volume=35):
    ...
    try:
        ...
        # 4. Clean up temporary MSCX file and MuseScore residues
        if os.path.exists(mscx_path):
            try:
                os.remove(mscx_path)
            except Exception:
...
def main():
    ...
    ap.add_argument("--output",     default="output/strings_16part", help="Pasta de saída para os arquivos MP3")
