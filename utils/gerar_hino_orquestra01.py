...
# -*- coding: utf-8 -*-
"""
utils/gerar_hino_orquestra01.py
================================
Orquestração de hino completo no estilo "Orquestra 01":
intercala frases usando dois agrupamentos:

  Agrupamento 1: STRINGS + BRASS + PALETAS + SOPROS (nicho puro)
  Agrupamento 2: COMBINATIONS_ORQUESTRA (mix de todas as famílias)

Padrão de frases (ex: 6 frases):
  F1: Agrupamento 1  (ex: um quarteto de cordas)
  F2: Agrupamento 2  (ex: Sinfônica Clássica)
  F3: Agrupamento 1  (ex: octeto de metais)
  F4: Agrupamento 2  (ex: Tutti Equilibrada)
  F5: Agrupamento 1  (ex: orq. de paletas 12 partes)
  F6: Agrupamento 2  (ex: Épica Sinfônica)  ← SEMPRE termina com Agrupamento 2

Saída: output/orquestra_nicho/orquestra01/hino_XXX.mp3 (+ _partes/)
"""
...
# ── Importar COMBINATIONS_ORQUESTRA do script de teste ──────────────────────
sys.path.insert(0, str(ROOT / 'scripts'))
from testar_orquestra_mix import COMBINATIONS_ORQUESTRA

# ── Agrupamentos ────────────────────────────────────────────────────────────
AGRUPAMENTO_1 = COMBINATIONS_STRINGS + COMBINATIONS_BRASS + COMBINATIONS_PALETAS + COMBINATIONS_SOPROS
AGRUPAMENTO_2 = COMBINATIONS_ORQUESTRA


def select_orquestra01_combos(n_phrases: int, seed: int = None) -> list:
    """
    Seleciona combos alternando entre Agrupamento 1 e 2.
    - Frases ímpares (1, 3, 5...): Agrupamento 1 (nicho puro)
    - Frases pares (2, 4, 6...): Agrupamento 2 (orquestra mix)
    ...
    # Decidir o padrão de alternância
    # starts_with = 1 se a primeira frase é Agrupamento 1
    # Se n_phrases == 1, usa direto Agrupamento 2
    if n_phrases == 1:
        config = _pick_by_size(AGRUPAMENTO_2, sizes[0])
        return [("AG2", config)]

    # Para garantir que a última frase é Agrupamento 2,
    # determinamos a sequência: AG1, AG2, AG1, AG2, ... AG2
    # Se n_phrases é par: AG1,AG2,AG1,AG2,...AG1,AG2 → OK, termina com AG2
    # Se n_phrases é ímpar: AG1,AG2,...AG1,AG2,AG2 → ajustamos
    selected = []
...
def escrever_explicacao_md(partes_dir: Path, selected: list, midi_name: str):
    ...
    lines = [
        f"# 🎹 Explicação dos Timbres e Dinâmicas — {midi_name} (Orquestra 01)",
        "",
        ...
        "- **Agrupamento 2**: Orquestra mista (combinação equilibrada de todas as famílias)",
    ...
    for f_idx, (grupo, config) in enumerate(selected):
        grupo_label = "Nicho Puro" if grupo == "AG1" else "Orquestra Mix"
...
def gerar_hino_orquestra01(midi_path: str, output_mp3: str, bpm_target: float = 60.0, speed: float = None, overwrite: bool = False) -> bool:
    """Gera um hino completo no estilo Orquestra 01 (intercalação AG1/AG2)."""
    ...
    print(f'\n{"="*60}')
    print(f'  HINO COMPLETO (ORQUESTRA 01): {Path(midi_path).name}')
    ...
    seed_val = hino_id + 7777  # Offset único para Orquestra 01

    selected = select_orquestra01_combos(len(phrases), seed=seed_val)
    print(f'\nProgressão dos Arranjos (ORQUESTRA 01):')
    for i, (grupo, config) in enumerate(selected):
        grupo_tag = "AG1-Nicho" if grupo == "AG1" else "AG2-Mix  "
    ...
    work_dir = Path(f'/tmp/_tmp_hino_orq01_{os.getpid()}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Gerar hino no estilo Orquestra 01 (intercalação nicho/mix)."
    )
    ...
    ok = gerar_hino_orquestra01(args.midi, args.out, args.bpm, args.speed)
