...
=========================
Script orquestrador para processar múltiplos hinos em lote, gerando para cada hino
4 versões de nicho (Strings, Brass, Paletas, Sopros) independentemente.
"""
...
def processar_lote(start_id: int = None, end_id: int = None, speed_factor: float = 1.0, overwrite: bool = False):
    ...
    print(f" Total de arquivos a gerar: {total} ({len(to_process)} hinos x {len(groups)} nichos)")
    ...
    for idx, (prefix, hino_id, midi_path) in enumerate(to_process, 1):
        for group in groups:
            ...
            if not overwrite and output_mp3.exists():
                print(f"\n[{idx_global}/{total}] Pulando {midi_path.name} ({group}) (já existe {output_mp3.name})")
                pulados += 1
                idx_global += 1
                continue
                
            print(f"\n[{idx_global}/{total}] Processando {midi_path.name} para o Nicho {group.upper()}...")
            ...
            t0 = time.time()
            try:
                ok = gerar_hino_nicho_completo(str(midi_path), str(output_mp3), group=group, bpm_target=bpm_target, speed=speed_param)
                t_elap = time.time() - t0
                if ok:
                    ...
                    falhas.append(f"{midi_path.name} ({group})")
            except Exception as e:
                t_elap = time.time() - t0
                print(f"  [EXCEÇÃO] Erro crítico no hino {hino_id} ({group}): {e}")
                falhas.append(f"{midi_path.name} ({group})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Geração de lote de nichos individuais para hinos.")
